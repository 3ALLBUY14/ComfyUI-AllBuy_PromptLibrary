...
const PLUGIN_VERSION = "v3.72"; // 改样式/逻辑时递增，用于强制浏览器刷新缓存（与后端 constants.PLUGIN_VERSION 一致）
...
function startController(node) {
  ...
  function renderItem(g, isSelected, list) {
    ...
    // v3.68：列表/卡片不再渲染封面缩略图/悬停播放——区域太小看不清，
    // 预览收敛到编辑弹窗（左媒体窗）与预览弹窗两处
    let item;
    if (state.view === "card") {
      ...
      item = h("div", {
        ...
        draggable: "true",
        "data-id": g.id,
      }, [dragHandle, dot, body, actions].filter(Boolean));
    }
